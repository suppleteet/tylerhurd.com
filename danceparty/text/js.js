...
$(document).ready( function() {
    ...
    build_page();
    bind_dance_modal();
    var hash_nav = window.location.hash.replace('#','');
    if (hash_nav) {
        open_dance_modal(hash_nav);
    }
});
...
function build_page() {
    $('#content').stop().animate( { "opacity":0 }, 5, function() {
        ...
        for( i=0; i<moves.length; i++ ) {
            the_html += '<li class="sub_nav" style="float: left;">'+
                    '<a href="#'+moves[i]+'" class="move-link" data-move="'+moves[i]+'">'+
                    '<div style="width:'+cell_width+'px; height:'+cell_height+'px; margin:'+cell_pad+'px '+(cell_pad*2)+'px '+cell_pad+'px 0px; overflow:hidden;">'+
                    '<div id="thumbDiv'+i+'" style="height:'+(cell_height)+'px; overflow:hidden;"></div></div></a></li>';
        }
...
function show_after_loaded() {
    $('#content').stop().animate( { 'opacity':1 }, 15 );
}

function bind_dance_modal() {
    $(document).on('click', '.move-link', function(e) {
        e.preventDefault();
    ...
    $('#dance-modal .modal-close, #dance-modal .modal-backdrop').on('click', function() {
        window.location.hash = '';
    });
    $('#dance-prev').on('click', function() {
        navigate_modal(-1);
    });
    $('#dance-next').on('click', function() {
        navigate_modal(1);
    });
...
function open_dance_modal(move) {
    actor_name = move.split('Moves')[0];
    movie_a = move+'_'+actor_name;
    movie_b = move;
    var media_folder = is_mobile ? 'mp4mobile' : 'mp4';

    var the_modal_html = '';
    the_modal_html += '<div class="media-item"><video class="dance_modal_video" autoplay loop muted playsinline poster="posters/'+movie_a+'.jpeg">';
    the_modal_html += '<source src="'+media_folder+'/'+movie_a+'.mp4" type=\'video/mp4;codecs="avc1.42E01E, mp4a.40.2"\'/></video></div>';
    the_modal_html += '<div class="media-item"><video class="dance_modal_video" autoplay loop muted playsinline poster="posters/'+movie_b+'.jpeg">';
    the_modal_html += '<source src="'+media_folder+'/'+movie_b+'.mp4" type=\'video/mp4;codecs="avc1.42E01E, mp4a.40.2"\'/></video></div>';

    $('#dance-modal-strip').html(the_modal_html);
    $('#dance-modal').removeClass('hidden closing').addClass('show').attr('aria-hidden', 'false');
}

function navigate_modal(direction) {
    var current_move = window.location.hash.replace('#','');
    var current_index = moves.indexOf(current_move);
    if (current_index === -1) {
        return;
    }
    var next_index = current_index + direction;
    if (next_index < 0 || next_index >= moves.length) {
        return;
    }
    window.location.hash = moves[next_index];
}

function close_dance_modal() {
    $('#dance-modal').removeClass('show').addClass('hidden').attr('aria-hidden', 'true');
    $('#dance-modal-strip video').each(function() {
        this.pause();
    });
    $('#dance-modal-strip').empty();
}
...
function add_thumbnail( move, par_div ) {
    ...
    // create hidden image
    $('#'+par_div).css('opacity','0.8').css('background-image','');
